...
object BitManipBFPOnlyPlugin {
	object BitManipBFPOnlyCtrlEnum extends SpinalEnum(binarySequential) {
		 val CTRL_BFP = newElement()
	}
	object BitManipBFPOnlyCtrl extends Stageable(BitManipBFPOnlyCtrlEnum())
// Prologue
...
class BitManipBFPOnlyPlugin(earlyInjection : Boolean = true) extends Plugin[VexRiscv] {
	import BitManipBFPOnlyPlugin._
	object IS_BitManipBFPOnly extends Stageable(Bool)
	object BitManipBFPOnly_FINAL_OUTPUT extends Stageable(Bits(32 bits))
	override def setup(pipeline: VexRiscv): Unit = {
		...
		decoderService.addDefault(IS_BitManipBFPOnly, False)
		decoderService.add(List(
			BFP_KEY	-> (binaryActions ++ List(BitManipBFPOnlyCtrl -> BitManipBFPOnlyCtrlEnum.CTRL_BFP))
		))
	} // override def setup
	override def build(pipeline: VexRiscv): Unit = {
		...
		execute plug new Area{
			import execute._
			insert(BitManipBFPOnly_FINAL_OUTPUT) := fun_bfp(input(SRC1), input(SRC2)).asBits
		} // execute plug newArea
		val injectionStage = if(earlyInjection) execute else memory
		injectionStage plug new Area {
			import injectionStage._
			when (arbitration.isValid && input(IS_BitManipBFPOnly)) {
				output(REGFILE_WRITE_DATA) := input(BitManipBFPOnly_FINAL_OUTPUT)
			} // when input is
